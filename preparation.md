...
- [x] **2.1 Create a single API key in the Anthropic Console**, shared by all 70 students and also used for the instructor's own prep runs and the centralized judge batch (§6.1, §7.2).
- [x] **2.2 Set a spend limit on the key**, sized for the full day: 70 students running both notebooks plus the centralized batch judge run. Measured input for the morning: one full run of the build notebook costs **$0.033**, so 70 students at three runs each is **under $7**. Session 2 is not what the cap needs to be sized for; the §7.2 judge batch will dominate. There's no per-key model restriction with a single shared key, so the notebooks themselves must pin students to Haiku by default; the spend cap is the backstop, not the primary control.
- [x] **2.3 Monitor total spend manually via the Anthropic Console usage dashboard** throughout the day. Because everyone shares one key, a single runaway loop affects the whole room, not just one group — keep the dashboard open continuously (see §10.1) and be ready to revoke/reissue the key if spend spikes.
- [x] **2.4 (Deferred — not for this run)** Per-student/per-group keys with individual spend limits and per-key model restriction, plus a LiteLLM proxy with live automated budget enforcement, would remove the single-point-of-failure and blast-radius risk of one shared key. Worth building for a second edition of the course; out of scope given this year's prep time.
...
- [x] **3.3 Set up a Slack channel with the master organizers** to distribute and, if needed, rapidly re-share a rotated key mid-day — with a single key covering the whole room, this is the fastest path to get a fresh key in front of 70 students at once if the original gets revoked or exhausted.
...
- [x] **5.4 `top_k_search` exercise**: vectorized cosine-similarity top-k + correction + test. Takes the query vector and the chunk matrix rather than issue #7's `(query: str, chunk_database: str)`, which keeps the inline check exact (a hand-built matrix, no model and no file) and keeps the function to the similarity itself. The `cosine_similarity` exercise issue #7 listed as optional is dropped: the vectors are normalised, so it would only restate the dot product.
- [x] **5.5 BM25 search + hybrid fusion — dropped.** The RAG should stay as simple as possible, so this only survives if it turns out cheap to add. Decide once §5.4 is done. Evidence gathered while writing §5.4, on the 7 questions whose reference answer carries a distinctive number: dense retrieval finds the answer chunk in 2/7 at k=5, a throwaway BM25 finds it in 5/7. The chunk reading "Revenue $ 215,938 $ 130,497 Up 65%" ranks 23rd of 922 for "What was NVIDIA's total revenue in fiscal year 2026?". Dense retrieval works on thematic questions and fails on table lookups. **Dropped anyway**: the guiding rule is the simplest RAG that works, and an extra retriever plus a fusion step is a second exercise in a session that is already full. The gap is a known limitation of this iteration rather than a problem to fix now.
- [x] **5.6 ReAct loop assembly exercise**: wire the search tool into a single simple ReAct agent (LangGraph or a manual loop — pick one, don't offer both) + correction + test. Hand-built LangGraph rather than the prebuilt `create_agent`. Two exercises rather than one: the tools (`search_filings` and `calculator`) and the graph itself. Departures from issue #7: the `State` is given rather than exercised, since its answer is looked up rather than reasoned out; `answer_question` is given; and the closing run over the questions is a guided cell rather than an exercise, because its answers cannot be asserted on.
- [x] **5.7 Full top-to-bottom dry run** of the notebook against the real API using the shared key, to validate both correctness and real token/cost consumption per run — this run is also what produces the agent used for the optional spot-check in §6.4. Measured on 2026-09-24 with the reference solutions in place: the notebook runs top to bottom in **43 seconds**, and its five agent calls consume **24,259 input / 1,659 output tokens, or $0.033 per run** at Haiku 4.5 rates. Embedding is local, so those five calls are the only paid work in the session. The artifact for §6.4 is deliberately not built yet, since it belongs to the afternoon material.
...
- [x] **6.1 Implement `instructor/generate_paragraph_claims.py`: a paragraph-level faithfulness dataset with ground truth known by construction, using two fixed prompts to a strong LLM (Opus) per chunk.** The corpus (§4.3) is far larger than this dataset needs, so sample a subset of chunks rather than processing all of them — that keeps the one-time Opus batch to the size and cost originally planned. For each sampled chunk:
  ...
  **Done as revised (2026-09-25).** Three departures from the text above. First, Claude Code subagents write the claims instead of an API script calling Opus, which keeps the shared key for students; the instructions are committed in `instructor/prompts/`. Second, a faithful claim is *deducible* from the chunk rather than a paraphrase of it. It may compare two stated figures but never compute a new one, and it is at most 20 words. Third, the dataset is two passes over the same 400 chunks (drawn with a fixed seed by `instructor/sample_chunks.py`): one faithful claim per chunk, then one unfaithful claim per chunk, which gives 800 claims whose faithful/unfaithful mix is chosen at the GLIDE step. The faithful pass is done and checked. `instructor/generate_paragraph_claims.py` merges the batches and runs the checks, and a second set of subagents then verified every claim against its chunk. They found 3 wrong claims out of 400, all corrected. Each unfaithful claim applies the one taxonomy category the writer found most natural for its chunk, and all ten categories are used (from 15 to 83 claims each). Verification found 4 wrong labels out of 400, all corrected.
- [ ] **6.2 Quality-pass the unfaithful claims.** Skim every generated "unfaithful" claim and make sure it's *subtly* wrong in the spirit of the taxonomy's Contresens/Troncature/Simplification examples above, not absurdly wrong — a claim the judge model would obviously catch 100% of the time produces no interesting bias to correct, which kills the "aha" moment in §7.4. Rewrite any claim that's too easy, too ambiguous, or doesn't cleanly match its intended category. **Done after the judge (PR 3):** its scores show which unfaithful claims are too easy to catch.
- [ ] **6.3 No build step needed for the labeled/proxy split — it's drawn live in the Session 4 notebook using a GLIDE sampler.** `paragraph_claims.json` already carries `true_faithfulness_label` for every id (fully labeled by construction), so unlike a real deployment there's no annotation cost to precompute or bake into a static file. The exercise itself calls a `glide.samplers` sampler (e.g. `StratifiedSampler`, stratifying on `error_type` so the ~20–30 revealed ids span multiple distortion categories rather than being skewed toward whichever category happened to generate first; `UniformSampler` if a simpler baseline is preferred) to pick which ids get their true label "revealed" as the labeled set — the rest fall back to judge-score-only (proxy). This is exactly the annotation-simulation use case `glide.samplers` is built for, and doing it live (rather than shipping a precomputed `labeled_subset_ids.json`) means students see GLIDE's sampling API in action, not just its estimators. **Nothing to build here; carried out in §7.3**, where the sampler choice still has to be checked against the installed `glide-py`.
...
- [x] **7.1 `score_faithfulness` exercise**: implement the LLM-as-judge call — given `(paragraph, claim)`, prompt the judge model and parse out a faithfulness score — + correction + test. **Done.** The correction is in PR #14 and the notebook exercise is on `feat/7.1-notebook-judge`. **We also added a second exercise, not in the original plan: `accuracy_per_class`**, which measures the judge's accuracy on faithful claims, on unfaithful claims and overall. Students use it first on a 10-claim test, then on all 800 claims, where the split (99.8% against 90%, for 95% overall) shows that the judge's errors all go one way. `score_faithfulness(chunk, claim, llm, system_prompt)` makes one Haiku call per claim and returns `{"reasoning", "verdict"}`, the verdict being 1 if the claim is deducible from its chunk. The prompt is given to students. It is adapted from Fig. 6 of Grégoire's paper (arXiv 2507.21753), zero-shot, asks for the reasoning first in under 100 words, and forbids outside knowledge. On all 800 claims the judge gets 399/400 faithful and 361/400 unfaithful claims right, so it is lenient: it puts the faithfulness rate at 54.8% where the truth is 50.0%, which is the bias GLIDE corrects.
- [x] **7.2 Implement `instructor/run_judge_scores.py`: precompute judge scores centrally.** Run `score_faithfulness` over every pair in `paragraph_claims.json` (§6.1 — now several hundred, since each paragraph yields one distortion per applicable category) yourself, once, using the shared key, and ship the results as `judge_scores.json`. Precomputing centrally — rather than having 70 students each call the judge on the same fixed data — protects the shared spend limit (§2.2) and guarantees everyone works from identical proxy labels; a few hundred one-time Opus calls is still cheap in absolute terms, since it's a single batch run, not 70 repetitions of it. Leave one or two cells where students make a *live* judge call themselves (on one pair of their choosing) purely so they've seen it happen, but the estimation exercise itself runs on the shipped data. **Done as revised on Grégoire's review of PR #14: students run the judge on all 800 claims themselves**, through the given `judge_all` in `utils/evaluate.py`. One run measured $0.90 and 2.7 minutes with 8 calls in parallel. The shared key allows 10,000 requests a minute and one student uses about 330, so only a whole room starting in the same minute would exceed it. The notebook will have students check their function on a few claims first. `instructor/run_judge_scores.py` stays as the instructor's tool: it measures the judge and produces reference scores in `data/06_claims/judge_scores.json`, for the §6.2 quality pass and as a fallback on the day.
