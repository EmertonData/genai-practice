...
- [ ] **5.5 BM25 search + hybrid fusion — likely dropped.** The RAG should stay as simple as possible, so this only survives if it turns out cheap to add. Decide once §5.4 is done. Evidence gathered while writing §5.4, on the 7 questions whose reference answer carries a distinctive number: dense retrieval finds the answer chunk in 2/7 at k=5, a throwaway BM25 finds it in 5/7. The chunk reading "Revenue $ 215,938 $ 130,497 Up 65%" ranks 23rd of 922 for "What was NVIDIA's total revenue in fiscal year 2026?". Dense retrieval works on thematic questions and fails on table lookups. To decide with Grégoire.
