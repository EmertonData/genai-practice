...
- **Session 2 — Build** (`notebooks/01_build_agentic_rag.ipynb`): students build a hybrid-search RAG agent over a financial document (chunking → embedding → vector search → BM25 → fusion → a ReAct agent loop).
