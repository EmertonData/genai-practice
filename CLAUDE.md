...
**Measured on the judge, so don't re-derive** (all 800 claims, 2026-09-25): 397/398 faithful claims judged correctly and 342/379 unfaithful ones, so the judge is lenient. It says 55.9% of claims are faithful where the truth is 51.2%. It misses mostly Simplification (35% caught) and Truncation (70%), and catches Reversal, Conflation, Invention, Agglomeration and Synecdoche at 100%. The run cost $0.84 and took 2.4 minutes with 8 calls in parallel (about 590 input and 93 output tokens per claim). 23 calls, 21 of them on unfaithful claims, have no verdict because `max_tokens=150` truncated long reasonings. Raise the cap to 400 at the next re-run, which PR 3 needs anyway. The shared key's limits are 10,000 requests, 10M input and 2M output tokens per minute: one student uses about 330 requests a minute, so 70 students starting in the same minute would need 2.3× the request limit. Don't add descriptions ahead of the actual exercise content — describe an exercise when it's authored.
